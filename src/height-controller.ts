 ...
 */
export function createHtmlArtifactHeightController(
  maxReportedHeight: number
): HtmlArtifactHeightController {
  const heightLimit =
    Number.isFinite(maxReportedHeight) && maxReportedHeight > 0
  ...
  const clampHeight = (value: number): number =>
    Math.max(1, Math.min(heightLimit, Math.ceil(Number(value) || 1)))
  const normalizeWidth = (value: number): number => Math.max(1, Math.ceil(Number(value) || 1))
  ...
  return {
    ...
    shouldPublish(size: HtmlArtifactPublishedSize): boolean {
      ...
      if (
        Math.abs(height - lastPublishedHeight) <= 1 &&
